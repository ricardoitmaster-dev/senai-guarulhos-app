...
with col2:
    ...
    with st.form("form_registro", clear_on_submit=True):
        ...
        whats = st.text_input("WhatsApp (com DDD)", key="whats_input")
        ...
        if enviar:
            if area_sel != "Selecione..." and nome and email:
                ...
                if salvar_novo_lead([nome, email, f"'{whats}", area_sel, curso_sel, obs, data_atual]):
                    st.success(f"Excelente, {nome}! Seu interesse foi registrado no sistema.")
                    st.balloons()
                    
                    # --- LÓGICA DO WHATSAPP ---
                    # Substitua pelo número da secretaria ou o seu para receber a mensagem
                    numero_destino = "551133220050" 
                    mensagem_texto = f"Olá! Acabei de me registrar pelo App. Tenho interesse no curso: *{curso_sel}*. Meu nome é *{nome}*."
                    ...
                    st.markdown(f"""
                        <a href="{link_wa}" target="_blank" class="btn-whatsapp">
                            <i class="fab fa-whatsapp" style="margin-right:10px;"></i> ENVIAR INTERESSE VIA WHATSAPP
                        </a>
                    """, unsafe_allow_html=True)
                    st.info("Clique no botão acima para confirmar sua pré-inscrição com nossa equipe.")
                else: st.error("Erro ao salvar no banco de dados.")
...
<div class="footer-container">
    ...
    <div class="footer-bottom">
        <span>SENAI GUARULHOS - UNIDADE 122</span>
    </div>
</div>
"""
...
with st.sidebar:
    ...
    if senha_digitada == senha_mestra and senha_mestra != "":
        ...
        if st.button("Sair", on_click=reset_geral_callback):
            st.rerun()
